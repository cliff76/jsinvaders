...
function buildFallingBlocks(gamePieces, numberOfBlocks) {
    ...
    locations.forEach(function (each) { return gameArea.gamePieces.push(buildFallingBlock(blockSize, each)); });
    setTimeout(buildFallingBlocks, 5000, gameArea.gamePieces, getRandomInt(1, 5));
}
function buildFallingBlock(blockSize, x) {
    var fallingBlock = new component(blockSize, blockSize, "red", x, 0);
...
function buildShip() {
    var sizeOfShip = { height: 30, width: 50 };
    var gapFromBottom = 10;
    var nearBottomOfScreen = gameArea.canvas.height - (sizeOfShip.height + gapFromBottom);
    var shipMiddleOfScreen = (gameArea.canvas.width / 2) - (sizeOfShip.width / 2);
    var ship = new component(sizeOfShip.width, sizeOfShip.height, "media/sprites/spaceship.png", shipMiddleOfScreen, nearBottomOfScreen, 'image');
    ship.direction = 0;
    ship.move = function () {
        var newX = ship.x + ship.direction;
...
function buildShot() {
    var ship = getShipComponent();
    var middleOfShip = (ship.x + (ship.width / 2));
    var shot = new component(5, 25, 'blue', middleOfShip - 2, gameArea.canvas.height - 50);
    shot.isShot = true;
    shot.move = function () {
...
function destroyComponent(component) {
    gameArea.gamePieces = gameArea.gamePieces.filter(function (it) { return it !== component; });
}
function component(width, height, nameOrColor, x, y, type) {
...
function gameLoop() {
    ...
    gameArea.gamePieces.forEach(function (eachPiece) {
        eachPiece.move();
        eachPiece.update();
    });
}
